...
export function Testimonials() {
  return (
    <section className="py-24 overflow-hidden relative border-y border-border/40 bg-background/50">
            {/* Background gradients */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 w-full h-[300px] bg-accent/5 blur-[120px] rounded-full pointer-events-none" />
          ...
          <div
            key={i}
            className="premium-card p-8 flex-shrink-0 w-[350px] md:w-[400px] flex flex-col justify-between hover-lift min-h-[260px] group">
            
                            {/* Top gradient accent */}
                            <div className={`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${COLORS[i % COLORS.length]} opacity-60`} />
                ...
                <Star key={j} className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />
                            ...
                            <div className="mt-6 flex items-center gap-3">
                                {/* Avatar */}
                                <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${COLORS[i % COLORS.length]} border border-border/30 flex items-center justify-center flex-shrink-0`}>
                                    <span className="text-xs font-semibold text-foreground/80">{testimonial.initials}</span>
                                </div>
                                <div>
                                    <p className="font-semibold text-foreground tracking-tight text-sm">{testimonial.name}</p>
                                    <p className="text-[11px] text-muted-foreground mt-0.5 uppercase tracking-widest">{testimonial.role}</p>
                                </div>
                            </div>
                        </div>
          )}
